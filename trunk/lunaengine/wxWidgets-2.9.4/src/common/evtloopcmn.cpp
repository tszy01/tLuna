    ...
    for ( ;; )
    {
        try
        {
			...
			for (;;)
			{
				// change by tang
				// 是否使用游戏循环
				bool bUseGameLoop = false;
				if (wxTheApp)
				{
					bUseGameLoop = wxTheApp->IsUseGameLoop();
				}
				// 根据是否使用游戏循环
				if (bUseGameLoop)
				{
					...
					if( ::PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) )
					{
						...
						if ( !DispatchForGameLoop(msg) )
						{
							break;
						}

						if(!wxTheApp->ProcessGameLoop())
						{
							break;
						}
					}
					...
					{
						// 如果App有ProcessIdle的要求，就让他做
						while(wxTheApp && wxTheApp->ProcessIdle())
						{
						}
						// if the "should exit" flag is set, the loop should terminate
						// but not before processing any remaining messages so while
				...
				else
				{
					...
					// if we has processed a msg
					// we call the function
					if(wxTheApp)
					{
						if(!wxTheApp->ProcessAppLoop())
						{
							break;
						}
					}
