...
namespace TLunaEngine{
	...
	public:
		...
		inline bool operator<(const TVector2<T>&other) const { return X<other.X && Y<other.Y; }
		inline bool operator>(const TVector2<T>&other) const { return X>other.X && Y>other.Y; }
		inline bool operator==(const TVector2<T>& other) const { return other.X==X && other.Y==Y; }
		// 不等于，有一个不等就返回真
		inline bool operator!=(const TVector2<T>& other) const { return other.X!=X || other.Y!=Y; }
		...
		inline T dotProduct(const TVector2<T>& other) const
		{
		...
		inline T getDistanceFrom(const TVector2<T>& other) const
		{
			return TVector2<T>(X - other.X, Y - other.Y).getLength();
		}
		...
		inline T getDistanceFromSQ(const TVector2<T>& other) const
		{
			return TVector2<T>(X - other.X, Y - other.Y).getLengthSQ();
		}

		//! rotates the point around a center by an amount of degrees.
		/** \param degrees Amount of degrees to rotate by.
		\param center Rotation center. */
		inline void rotateBy(TF64 degrees, const TVector2<T>& center)
		{
			degrees *= DEGTORAD64;
			T cs = (T)cos(degrees);
			T sn = (T)sin(degrees);
		...
		inline TF64 getAngleWith(const TVector2<T>& b) const
		{
		...
		inline bool isBetweenPoints(const TVector2<T>& begin, const TVector2<T>& end) const
		{
		...
		inline TVector2<T> getInterpolated(const TVector2<T>& other, TF32 d) const
		{
			TF32 inv = 1.0f - d;
			return TVector2<T>(other.X*inv + X*d, other.Y*inv + Y*d);
		}
		...
		\param d Value between 0.0f and 1.0f.
		\return Interpolated vector. */
		inline TVector2<T> getInterpolated_quadratic(const TVector2<T>& v2, const TVector2<T>& v3, const TF32 d) const
		{
			...
			return TVector2<T> ( X * mul0 + v2.X * mul1 + v3.X * mul2,
		...
		inline void interpolate(const TVector2<T>& a, const TVector2<T>& b, const TF32 t)
		{
			X = b.X + ( ( a.X - b.X ) * t );
			Y = b.Y + ( ( a.Y - b.Y ) * t );
		}
	...
	typedef TVector2<TF32> TVecFloat2;
	// define double vector2
	typedef TVector2<TF64> TVecDouble2;
	// define int vector2
	typedef TVector2<TS32> TVecInt2;
	// define uint vector2
	typedef TVector2<TU32> TVecUInt2;
