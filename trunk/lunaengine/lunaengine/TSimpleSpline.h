...
namespace TLunaEngine
{
	...
	public:
        ...
        inline void addPoint(const TVector3<T>& p)
		{
			mPoints.push_back(p);
			if (mAutoCalc)
			{
        ...
        inline const TVector3<T>& getPoint(TU16 index) const
		{
        ...
        */
        inline void updatePoint(TU16 index, const TVector3<T>& value)
		{
			assert (index < mPoints.size() && "Point index is out of bounds!!");
        ...
        */
        inline TVector3<T> interpolate(TF32 t) const
		{
        ...
        @param t Parametric value
        */
        inline TVector3<T> interpolate(TU32 fromIndex, TF32 t) const
		{
			// Bounds check
			...
			t2 = (T)t * (T)t;
			t3 = t2 * (T)t;
			TVector4<T> powers(t3, t2, t, 1);


			// Algorithm is ret = powers * mCoeffs * TMatrix4x4<T>(point1, point2, tangent1, tangent2)
			const TVector3<T>& point1 = mPoints[fromIndex];
			const TVector3<T>& point2 = mPoints[fromIndex+1];
			const TVector3<T>& tan1 = mTangents[fromIndex];
			const TVector3<T>& tan2 = mTangents[fromIndex+1];
			TMatrix4x4<T> pt;
			...
			TVector4<T> ret = powers * mCoeffs * pt;


			return TVector3<T>(ret.x, ret.y, ret.z);
		}
    ...
    protected:
        bool mAutoCalc;
		std::vector<TVector3<T>> mPoints;
        std::vector<TVector3<T>> mTangents;
        /// Matrix of coefficients 
        TMatrix4x4<T> mCoeffs;
	};

	// define float simple spline
	typedef TSimpleSpline<TF32> TSimpleSplineFloat;
	// define double simple spline
	typedef TSimpleSpline<TF64> TSimpleSplineDouble;
	// define int simple spline
	typedef TSimpleSpline<TS32> TSimpleSplineInt;
	// define uint simple spline
	typedef TSimpleSpline<TU32> TSimpleSplineUInt;
}