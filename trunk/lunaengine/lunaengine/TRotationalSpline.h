...
namespace TLunaEngine
{
	...
	public:
        ...
        inline void addPoint(const TQuaternion<T>& p)
		{
			mPoints.push_back(p);
			if (mAutoCalc)
			{
        ...
        inline const TQuaternion<T>& getPoint(TU16 index) const
		{
        ...
        */
        inline void updatePoint(TU16 index, const TQuaternion<T>& value)
		{
			assert (index < mPoints.size() && "Point index is out of bounds!!");
			mPoints[index] = value;
			if (mAutoCalc)
        ...
        */
        inline TQuaternion<T> interpolate(TF32 t, bool useShortestPath=true)
		{
		...
		@param useShortestPath Defines if rotation should take the shortest possible path
        */
        inline TQuaternion<T> interpolate(TU32 fromIndex, TF32 t, bool useShortestPath=true)
		{
			// Bounds check
			...
			TQuaternion<T> &p = mPoints[fromIndex];
			TQuaternion<T> &q = mPoints[fromIndex+1];
			TQuaternion<T> &a = mTangents[fromIndex];
			TQuaternion<T> &b = mTangents[fromIndex+1];
        ...
        */
        inline void recalcTangents(void)
		{
			...
			TQuaternion<T> invp, part1, part2, preExp;
			for(i = 0; i < numPoints; ++i)
			{
				TQuaternion<T> &p = mPoints[i];
				invp = p.Inverse();
    ...
    protected:
        bool mAutoCalc;
		std::vector<TQuaternion<T>> mPoints;
        std::vector<TQuaternion<T>> mTangents;
	};

	// define float rotational spline
	typedef TRotationalSpline<TF32> TRotationalSplineFloat;
	// define double rotational spline
	typedef TRotationalSpline<TF64> TRotationalSplineDouble;
	// define int rotational spline
	typedef TRotationalSpline<TS32> TRotationalSplineInt;
	// define uint rotational spline
	typedef TRotationalSpline<TU32> TRotationalSplineUInt;
}