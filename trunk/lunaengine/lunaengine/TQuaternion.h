...
namespace TLunaEngine{
	...
	public:
		...
		//! Constructor which converts euler angles (radians) to a TQuaternion
		TQuaternion(T x, T y, T z){set(x,y,z);}

		//! Constructor which converts euler angles (radians) to a TQuaternion
		TQuaternion(const TVector3<T>& vec){set(vec.X,vec.Y,vec.Z);}

		////! Constructor which converts a matrix to a TQuaternion
		...
		//! Equalilty operator
		inline bool operator==(const TQuaternion<T>& other) const
		{
			if(X != other.X)
				return false;
			if(Y != other.Y)
		...
		//! Assignment operator
		inline TQuaternion<T>& operator=(const TQuaternion<T>& other)
		{
			X = other.X;
			Y = other.Y;
			Z = other.Z;
		...
		//inline TQuaternion<T>& operator=(const TMatrix4x4<T>& m)
		//{
		//	T diag = m[0][0] + m[1][1] + m[2][2] + 1;
		//	TF32 scale = 0.0f;
		...
		//! Add operator
		inline TQuaternion<T> operator+(const TQuaternion<T>& other) const
		{
			return TQuaternion<T>(X+other.X, Y+other.Y, Z+other.Z, W+other.W);
		}
		...
		//! Multiplication operator
		inline TQuaternion<T> operator*(T s) const
		{
			return TQuaternion<T>(s*X, s*Y, s*Z, s*W);
		}

		//! Multiplication operator
		inline TQuaternion<T>& operator*=(T s)
		{
			X *= s; Y*=s; Z*=s; W*=s;
			return *this;
		}
		...
		inline T getDotProduct(const TQuaternion<T>& other) const
		{
			return (X * other.X) + (Y * other.Y) + (Z * other.Z) + (W * other.W);
		}

		//! Sets new TQuaternion
		...
		//! Sets new TQuaternion based on euler angles (radians)
		...
		//! Sets new TQuaternion based on euler angles (radians)
		inline void set(const TVector3<T>& vec)
		{
			set(vec.X, vec.Y, vec.Z);
		}

		//! Normalizes the TQuaternion
		...
		////! Creates a matrix from this TQuaternion
		//inline TMatrix4x4<T> getMatrix() const
		//{
		//	TMatrix4x4<T> m;
		...
		////! Creates a matrix from this TQuaternion
		//inline void getMatrix( TMatrix4x4<T> &dest ) const
		//{
		...
		////! Creates a matrix from this TQuaternion
		//inline void getMatrix_transposed( TMatrix4x4<T> &dest ) const
		//{
		...
		inline TQuaternion<T>& makeInverse()
		{
			X = -X; Y = -Y; Z = -Z;
			return *this;
		}

		//! Set this TQuaternion to the result of the interpolation between two quaternions
		inline void slerp(const TQuaternion<T>& q1,const TQuaternion<T>& q2, TF32 interpolate )
		{
        ...
        inline void squad (TF32 fT, const TQuaternion<T>& rkP,
            const TQuaternion<T>& rkA, const TQuaternion<T>& rkB,
            const TQuaternion<T>& rkQ/*, bool shortestPath = false*/)
		{
			TF32 fSlerpT = 2.0f*fT*(1.0f-fT);
			TQuaternion<T> kSlerpP;
			kSlerpP.slerp(rkP,rkQ,fT/*,shortestPath*/);
			TQuaternion<T> kSlerpQ;
			kSlerpQ.slerp(rkA,rkb,fT);
			slerp(kSlerpP,kSlerpQ,fSlerpT);
		}

		//! Create TQuaternion from rotation angle and rotation axis.
		/** Axis must be unit length.
		The TQuaternion representing the rotation is
		q = cos(A/2)+sin(A/2)*(x*i+y*j+z*k).
		\param angle Rotation Angle in radians.
		\param axis Rotation axis. */
		inline void fromAngleAxis (T angle, const TVector3<T>& axis)
		{
		...
		inline void toAngleAxis (T &angle, TVector3<T>& axis) const
		{
		...
		//! Output this TQuaternion to an euler angle (radians)
		inline void toEuler(TVector3<T>& euler) const
		{
			T sqw = W*W;
			T sqx = X*X;
			T sqy = Y*Y;
		...
		//! Set TQuaternion to represent a rotation from one vector to another.
		inline void rotationFromTo(const TVector3<T>& from, const TVector3<T>& to)
		{
			...
			TVector3<T> c = v0.crossProduct(v1);

			T d = v0.dotProduct(v1);
			if (d >= (T)1.0f) // If dot == 1, vectors are the same
			{
				*this=TQuaternion<T>(0,0,0,1); //IDENTITY;
			}
		...
		inline TQuaternion<T> exp () const
		{
        ...
        inline TQuaternion<T> log () const
		{
	...
	typedef TQuaternion<TF32> TQuaternionFloat;
	// define double Quaternion
	typedef TQuaternion<TF64> TQuaternionDouble;
	// define int Quaternion
	typedef TQuaternion<TS32> TQuaternionInt;
	// define uint Quaternion
	typedef TQuaternion<TU32> TQuaternionUInt;
}