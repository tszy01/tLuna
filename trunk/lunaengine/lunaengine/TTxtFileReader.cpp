...
namespace TLunaEngine{
	...
	TTxtFileReader::TTxtFileReader(void)
	{
	}
	...
	bool TTxtFileReader::OpenTxtFile(const char *filename, FILE **ppStream)
	{
	...
	void TTxtFileReader::CloseTxtFile(FILE *pStream)
	{
		fclose(pStream);
	}

	bool TTxtFileReader::ReadLineString(char *strResult, FILE *pStream, char *strCmp, bool *bEqual, int nCount, int *pReadCount)
	{
	...
	bool TTxtFileReader::ReadLineInteger(int *aiResult, FILE *pStream, int nCount, char splice)
	{
	...
	bool TTxtFileReader::ReadLineLong(long* alResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadLineShort(short* asResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadLineUInteger(unsigned int* auResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadLineULong(unsigned long* aulResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadLineUShort(unsigned short* ausResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadLineFloat(float *afResult, FILE *pStream, int nCount, char splice)
	{
	...
	bool TTxtFileReader::ReadLineDouble(double* adResult,FILE* pStream,int nCount,char splice)
	{
	...
	bool TTxtFileReader::ReadAllFile(const char* szFile,const char* mode,void** buffer,size_t* readByte)
	{
		if(!szFile || !mode || !buffer || !readByte || (*buffer)!=NULL)
		{
			return false;
		}
		FILE* file = 0;
