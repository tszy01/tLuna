...
{
	template<typename T>
	class TPlaneBoundedVolume
	{
	public:
        /// Publicly accessible plane list, you can modify this direct
		std::vector<TPlane<T>> planes;
        EIntersectionRelation3D outside;

		TPlaneBoundedVolume() :outside(ISREL3D_BACK) {}
        /** Constructor, determines which side is deemed to be 'outside' */
        TPlaneBoundedVolume(EIntersectionRelation3D theOutside) 
            : outside(theOutside) {}

        /** Intersection test with AABB
        @remarks May return false positives but will never miss an intersection.
        */
        inline bool intersects(const TVector3<T>& center, const TVector3<T>& halfSize) const
        {
			std::vector<TPlane<T>>::iterator itr = planes.begin();
			for (;itr != planes.end(); ++itr)
            {
                const TPlane<T>& plane = *itr;
        ...
        @remarks May return false positives but will never miss an intersection.
        */
        inline bool intersects(const TVector3<T>& center, T radius) const
        {
            std::vector<TPlane<T>>::iterator itr = planes.begin();
			for (;itr != planes.end(); ++itr)
            {
        ...
        @remarks May return false positives but will never miss an intersection.
        */
        inline std::pair<bool, TF32> intersects(const TVector3<T>& point, const TVector3<T>& dir)
        {
