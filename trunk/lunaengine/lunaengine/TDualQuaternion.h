...
namespace TLunaEngine{
	...
	public:
		...
		TDualQuaternion (T fW, T fX, T fY, T fZ, 
		...
		inline TDualQuaternion<T>& operator= (const TDualQuaternion<T>& rkQ)
		{
			w = rkQ.w;
			x = rkQ.x;
			y = rkQ.y;
		...
		inline bool operator== (const TDualQuaternion<T>& rhs) const
		{
		...
		void fromRotationTranslation (const TQuaternion<T>& q, const TVector3<T>& trans)
		{
		...
		void toRotationTranslation (TQuaternion<T>& q, TVector3<T>& translation) const
		{
