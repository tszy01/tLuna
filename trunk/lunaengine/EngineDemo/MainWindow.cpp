		...
		case WM_SIZE:
			{
				...
				MainWindow::getSingletonPtr()->onSizeChanged(width,height);
			}
			break;
		default:
...
MainWindow::MainWindow(void) : m_hInst(NULL),m_hWnd(NULL),m_widthPro(0),m_heightPro(0),m_bWnd(false)
{
}
...
HRESULT MainWindow::InitWindow(HINSTANCE hInst, int nCmdShow, const _MAINWNDCONFIG* config)
{
	if(!config)
		return E_FAIL;
	WNDCLASS  wc;    //定义窗口类
...
void MainWindow::onSizeChanged(UINT width, UINT height)
{
	CalcRectPro();
}

void MainWindow::GetCursorPosInBuffer(float *x, float *y)
{
	if(!x || !y)
		return;
	POINT pt;
